...
I would like to look at trends over different generations, to see if we can find any notable trends. Let's first start of with finding the mean HP of every generation, and comparing those.
```{r}
average_HP <- pokemon %>%
...
In Pokemon, there are three starter Pokemon in every game. There is always a different set of three Pokemon, but there is always one Fire type, one Water type, and one Grass type. Instead of looking at every type of Pokemon in the game, I will be focusing on these three for this reason. My null hypothesis is that there is no significant change . So far, we have only been plotting bar plots, so this will be a good change. We will now be plotting scatter plots.

## Machine Learning

I will now be using a linear regression model to
